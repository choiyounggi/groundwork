...
| D11 | watch supervision | SHIPPED: `watch-status.sh` globs `<escdir>/*.json` and exits 5 to wake the coordinator immediately (solves the 1h-block). DEFERRED (need worker↔session mapping / a phase setter not in this pass, and the escalation glob already solves the core block): `tmux has-session` liveness and a `waiting` phase + `loop-gate` change | orca's attention routing, files-only |
