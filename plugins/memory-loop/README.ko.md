...
- **HABITS.md** — 교정과 사고를 상시 행동으로 바꾸는 증류 프레임. 두 파일로 나뉩니다 — 규칙은 매 세션 로드되고, 그 규칙이 나온 사례 기록은 `HABITS-CASES.md`에 두어 `[Cnn]` 포인터로 필요할 때만 읽습니다. 항상 로드되는 파일은 매 요청마다 다시 읽히므로 작게 유지해야 합니다.
  (긍정 프랙티스 🟢, 하드 라인 🛑).
...
| `habitsSplitWarnBytes` | `40000` | HABITS.md가 이 크기를 넘으면 넛지가 배경 산문을 `HABITS-CASES.md`로 옮기도록 함께 안내 (`0`이면 비활성) |
